...
def load_known():
    df=pd.read_csv(os.path.join(CLEAN_DIR,"表单2_填0版_全量58.csv"),encoding="utf-8-sig")
    df=df[df["是否有效"]].copy()
    df["类型编码"]=df["类型"].map(TYPE_MAP)
    return df
def load_unknown():
    df=pd.read_csv(os.path.join(CLEAN_DIR,"表单3_未知_clean.csv"),encoding="utf-8-sig").copy()
    for c in COMPONENTS:
...
def fit_discriminators(known):
    ...
    for tr,va in skf.split(X,y):
        lr_acc.append(Pipeline([("scale",StandardScaler()),("lr",LogisticRegression(max_iter=2000,random_state=RANDOM_STATE))]).fit(X[tr],y[tr]).score(X[va],y[va]))
        rf_acc.append(RandomForestClassifier(n_estimators=500,random_state=RANDOM_STATE).fit(X[tr],y[tr]).score(X[va],y[va]))
        dt_acc.append(DecisionTreeClassifier(max_depth=3,min_samples_leaf=2,random_state=RANDOM_STATE).fit(X[tr],y[tr]).score(X[va],y[va]))
    scaler=StandardScaler()
    lr=LogisticRegression(max_iter=2000,random_state=RANDOM_STATE)
    rf=RandomForestClassifier(n_estimators=500,random_state=RANDOM_STATE)
    dt=DecisionTreeClassifier(max_depth=3,min_samples_leaf=2,random_state=RANDOM_STATE)
    lr.fit(scaler.fit_transform(X),y)
    rf.fit(X,y)
    dt.fit(X,y)
    disc={"scaler":scaler,"lr":lr,"rf":rf,"dt":dt}
    cv={"lr":np.mean(lr_acc),"rf":np.mean(rf_acc),"dt":np.mean(dt_acc)}
    print("  判别底座 分层5折CV: Logistic {:.4f} / RF {:.4f} / 决策树 {:.4f}".format(cv["lr"],cv["rf"],cv["dt"]))
    return disc,cv
def fit_preweather_models(df):
    ...
    for t in["高钾","铅钡"]:
        ...
        for c in MODEL_COMPS[t]:
            if c!="SiO2":
                denom+=b[c]*ratio[c]
        models[t]={"coef":b,"ratio":ratio,"denom":denom,"n":len(sub),"R2":m.rsquared}
        print("  风化还原[{}] R²={:.4f} 反推分母={:.4f}（n={}）".format(t,m.rsquared,denom,len(sub)))
    return models
...
def predict_prob(vec,disc):
    ...
    return {"p_lr":float(p_lr),"p_rf":float(p_rf),"p_dt":float(p_dt),"votes":votes}
def majority(votes):
    return "铅钡" if votes.count("铅钡")>=2 else "高钾"
def perturbation_sensitivity(vec,disc):
    rng=np.random.default_rng(RANDOM_STATE)
    vec=np.asarray(vec,dtype=float)
    base=disc["lr"].predict_proba(disc["scaler"].transform(vec.reshape(1,-1)))[0,1]
    out={}
    for amp in(0.05,0.10):
        flips=0
        for _ in range(N_REPEAT):
            vec_p=vec*rng.normal(1.0,amp,size=len(vec))
            p=disc["lr"].predict_proba(disc["scaler"].transform(vec_p.reshape(1,-1)))[0,1]
            if(p>0.5)!=(base>0.5):
                flips+=1
...
def main():
    ...
    for _,r in unknown.iterrows():
        ...
        if weathered:
            for hyp in["高钾","铅钡"]:
                ...
                recons[hyp]=pre
                if pre is None:
                    print("  [{}] {}假设还原：关键成分缺失或超出物理范围 → 不可行".format(wid,hyp))
                    continue
                pp=predict_prob(pre,disc)
                hyp_cls=majority(pp["votes"])
                cand.append((hyp_cls,pp["p_lr"],"{0}假设还原(判为{1})".format(hyp,hyp_cls)))
                print("  [{}] {}假设还原 → 判为{}（P(铅钡)={:.4f}，投票 {}）".format(wid,hyp,hyp_cls,pp["p_lr"],"/".join(pp["votes"])))
        ...
        rows.append({"样品":wid,"表面风化":r["表面风化"],"直接判别P(铅钡)":round(p_direct["p_lr"],4),"高钾假设还原P(铅钡)":round(predict_prob(recons["高钾"],disc)["p_lr"],4) if weathered and recons.get("高钾") is not None else "","铅钡假设还原P(铅钡)":round(predict_prob(recons["铅钡"],disc)["p_lr"],4) if weathered and recons.get("铅钡") is not None else "","最终类型":final_cls,"置信度":round(abs(final_p-0.5)*2,4),"判别依据":basis,})
        sens_rows.append({"样品":wid,"表面风化":r["表面风化"],"最终类型":final_cls,"±5%扰动翻转率":round(flip[0.05],4),"±10%扰动翻转率":round(flip[0.10],4),})
        print("  [{}] 表面{} 直接判别={}（P(铅钡)={:.4f}，投票 {}）→ 最终类型={}（依据：{}，置信度 {:.4f}）".format(wid,r["表面风化"],cls_direct,p_direct["p_lr"],"/".join(p_direct["votes"]),final_cls,basis,abs(final_p-0.5)*2))
    tab4=pd.DataFrame(rows)
    tab5=pd.DataFrame(sens_rows)[["样品","±5%扰动翻转率","±10%扰动翻转率"]]
    stage(4,5,"敏感性汇总与核对")
    print("  扰动翻转率：全部为 {}（论文表5）".format("0" if(tab5["±5%扰动翻转率"].astype(str)=="0.0").all() and(tab5["±10%扰动翻转率"].astype(str)=="0.0").all() else "非零，见下表"))
    print(tab5.to_string(index=False))
    stage(5,5,"生成论文插图（PDF）并写出 第三问.xlsx")
    order=pd.concat([tab4[tab4["最终类型"]=="高钾"].sort_values("直接判别P(铅钡)"),tab4[tab4["最终类型"]=="铅钡"].sort_values("直接判别P(铅钡)"),]).reset_index(drop=True)
    ...
    ax.text(0.5,len(ids)-0.30,"0.5 判界",color="#D9534F",fontsize=9,ha="center",va="bottom")
    for yi,p,pid in zip(y,probs,ids):
        ax.text(p+0.012,yi,"{:.2f}".format(p),va="center",fontsize=9,color="#333")
        ax.text(-0.012,yi,pid,va="center",ha="right",fontsize=9,color="#333",fontweight="bold")
    ...
    ax.legend(handles=[Patch(color="#E8A13D",label="高钾"),Patch(color="#5B8FF9",label="铅钡")],loc="lower right",fontsize=9,frameon=False,ncol=2)
    plt.tight_layout()
    ...
    wb.save(OUT_XLSX)
    print("\n问题 3 完成：输出 {}；图 → {} （总耗时 {:.1f}s）".format(OUT_XLSX,FIG_DIR,time.time()-t_all))
