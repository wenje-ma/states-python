...
ALR_REF="SiO2"
SVR_PARAM_GRID={"C":[0.1,1,10,50],"gamma":[0.01,0.05,0.1,0.5],"epsilon":[0.01,0.05,0.1]}
_t0=time.time()
...
def save_derived(table,tag):
    ...
    flags.to_csv(os.path.join(OUT_DIR,"表单2_检出标志{}.csv".format(tag)),index=False,encoding="utf-8-sig")
    zf=zero_filled(table)
    zf.to_csv(os.path.join(OUT_DIR,"表单2_填0版{}.csv".format(tag)),index=False,encoding="utf-8-sig")
    ...
    logratio.to_csv(os.path.join(OUT_DIR,"表单2_对数比变换{}.csv".format(tag)),index=False,encoding="utf-8-sig")
...
def svr_impute_all(valid):
    ...
    print("  有效文物样本: {} 件 | 特征(低缺失): {} | 插补目标(中缺失): {}".format(len(valid),features,targets))
    ...
    for col in targets:
        ...
        gs=GridSearchCV(SVR(kernel="rbf"),SVR_PARAM_GRID,cv=5,scoring="neg_mean_absolute_error")
        gs.fit(X_tr_s,y_tr)
        ...
        ev={"MAE":mean_absolute_error(y_true_all,y_pred_all),"RMSE":float(np.sqrt(mean_squared_error(y_true_all,y_pred_all))),"R2":r2_score(y_true_all,y_pred_all),"best_params":gs.best_params_,"n_known":int(known.sum()),"n_missing":int((~known).sum())}
        imputed=valid[col].copy()
        if(~known).sum()>0:
            X_te_s=scaler.transform(X_te)
            imputed.loc[~known]=best.predict(X_te_s)
        out[col+"_SVR插补"]=imputed
        report_rows.append({"成分":col,"缺失率%":round(miss_rate[col],1),"已检出数":ev["n_known"],"插补数":ev["n_missing"],"CV_MAE":round(ev["MAE"],3),"CV_RMSE":round(ev["RMSE"],3),"CV_R2":round(ev["R2"],3),"最优参数":str(ev["best_params"])})
        print("    [{}] 缺失率 {:.1f}% | 已检出 {} / 插补 {} | 5折CV R2={:.3f} | 参数 {}".format(col,miss_rate[col],ev["n_known"],ev["n_missing"],ev["R2"],ev["best_params"]))
...
def full_artifact_table(f1,f2):
    ...
    art=art.merge(f1[["文物编号","类型","纹饰","颜色","表面风化"]],on="文物编号",how="left")
    art["成分累加和"]=art[COMPONENTS].sum(axis=1)
    art["检出成分数"]=art[COMPONENTS].notna().sum(axis=1)
    art["是否有效"]=art["成分累加和"].between(SUM_MIN,SUM_MAX)
    return art
...
def q1_component_table(f2):
    f2=f2.copy()
    f2["风化状态"]=np.select([f2["采样点类型"]=="未风化点",f2["采样点类型"]=="严重风化点",f2["表面风化"]=="无风化"],["未风化","严重风化","无风化文物"],default="风化文物")
    return f2
def q1_restore_table(art,special):
    ...
    valid["风化程度"]=np.select([valid["表面风化"]=="无风化",valid["文物编号"].isin(severe_ids)],[0,2],default=1)
    zf=valid.copy()
    zf[COMPONENTS]=zf[COMPONENTS].fillna(0.0)
    ref="SiO2"
    zf2=zf.copy()
    ...
    out=valid[["文物编号","类型","表面风化","风化程度","成分数据来源","成分累加和","是否有效"]].copy()
    for col in COMPONENTS:
...
def main():
    ...
    f2=load_form2()
    f3=load_form3()
    print("  表单1 {} 件 | 表单2 {} 个采样点（有效 {}）| 表单3 {} 件（有效 {}）".format(len(f1),len(f2),f2["是否有效"].sum(),len(f3),f3["是否有效"].sum()))
    f1.to_csv(os.path.join(OUT_DIR,"表单1_基本信息_clean.csv"),index=False,encoding="utf-8-sig")
    f2.to_csv(os.path.join(OUT_DIR,"表单2_成分_clean.csv"),index=False,encoding="utf-8-sig")
    f3.to_csv(os.path.join(OUT_DIR,"表单3_未知_clean.csv"),index=False,encoding="utf-8-sig")
    ...
    art.to_csv(os.path.join(OUT_DIR,"表单2_文物级_clean.csv"),index=False,encoding="utf-8-sig")
    full.to_csv(os.path.join(OUT_DIR,"表单2_文物级_全量58.csv"),index=False,encoding="utf-8-sig")
    special.to_csv(os.path.join(OUT_DIR,"表单2_特殊点_clean.csv"),index=False,encoding="utf-8-sig")
    print("  文物级主表 {} 件（有效 {}）| 全量 58 件（有效 {}）| 特殊点 {} 条".format(len(art),art["是否有效"].sum(),len(full),full["是否有效"].sum(),len(special)))
    stage(3,4,"SVR 插补（K2O / PbO / BaO）")
    valid=art[art["是否有效"]].copy()
    out_svr,ev_df=svr_impute_all(valid)
    out_svr.to_csv(os.path.join(OUT_DIR,"表单2_文物级_SVR插补.csv"),index=False,encoding="utf-8-sig")
    ev_df.to_csv(os.path.join(OUT_DIR,"SVR插补评估表.csv"),index=False,encoding="utf-8-sig")
    stage(4,4,"问题 1 准备表")
    art_q1=full_artifact_table(f1,f2)
    art_q1.to_csv(os.path.join(OUT_DIR,"表单2_文物级_全量.csv"),index=False,encoding="utf-8-sig")
    q1_category_table(art_q1,f2).to_csv(os.path.join(Q1_DIR,"Q1_类别属性表.csv"),index=False,encoding="utf-8-sig")
    q1_component_table(f2).to_csv(os.path.join(Q1_DIR,"Q1_成分对照表.csv"),index=False,encoding="utf-8-sig")
    q1_restore_table(art_q1,special).to_csv(os.path.join(Q1_DIR,"Q1_风化反推表.csv"),index=False,encoding="utf-8-sig")
