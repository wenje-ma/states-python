...
ELEM_SHORT = {"SiO2": "硅", "PbO": "铅", "P2O5": "磷", "CuO": "铜",
              "Al2O3": "铝", "K2O": "钾", "CaO": "钙", "BaO": "钡",
              "Na2O": "钠", "Fe2O3": "铁", "SrO": "锶", "SnO2": "锡"}
PAPER_SUBCLASS_ORDER = [
    ("高钾", "高钙型"), ("高钾", "低钙型"), ("高钾", "高钡型"), ("高钾", "高铅型"),
    ("铅钡", "高硅型"), ("铅钡", "高铅型"), ("铅钡", "高磷型"), ("铅钡", "高铜型"),
    ("铅钡", "高铝型"), ("铅钡", "高钾型"),
]
...
def rf_importance(df):
    ...
    imp_df = pd.DataFrame({"成分": COMPONENTS, "特征重要性": np.round(imp, 4)}
...
def decision_tree_rules(df):
    ...
    skf = StratifiedKFold(n_splits=N_SPLITS, shuffle=True, random_state=RANDOM_STATE)
    accs = [DecisionTreeClassifier(max_depth=3, min_samples_leaf=2,
                                   random_state=RANDOM_STATE)
            .fit(X[tr], y[tr]).score(X[va], y[va]) for tr, va in skf.split(X, y)]
...
def logistic_discriminant(df):
    ...
    coef_df = pd.DataFrame({"成分": COMPONENTS, "判别系数": np.round(coef, 4)}
                           ).sort_values("判别系数", key=abs, ascending=False
                                         ).reset_index(drop=True)
...
def analysis_61(df):
    ...
    tab2 = pd.DataFrame({
        "成分": top8,
        "Spearman ρ": [corr.loc[corr["成分"] == c, "Spearman相关系数"].iloc[0] for c in top8],
        "RF 特征重要性": [imp_df.loc[imp_df["成分"] == c, "特征重要性"].iloc[0] for c in top8],
        "Logistic 判别系数": [coef_df.loc[coef_df["成分"] == c, "判别系数"].iloc[0] for c in top8],
    })
    top7_cum = imp_df["累计重要性"].iloc[6]
    print("  Spearman 相关前8: " + ", ".join(
        "{}ρ={:+.3f}".format(c, r) for c, r in zip(corr["成分"].head(8), corr["Spearman相关系数"].head(8))))
    print("  RF 前7位累计重要性 {:.4f}（论文 91.6%）| 5折CV准确率 {:.4f}±{:.4f}".format(
        top7_cum, rf_acc, rf_std))
    print("  决策树 5折CV {:.4f}±{:.4f} | 规则：\n{}".format(dt_acc, dt_std, rules))
    print("  Logistic 5折CV {:.4f}±{:.4f} | 系数前8: {}".format(lr_acc, lr_std, ", ".join(
        "{}={:+.3f}".format(c, v) for c, v in zip(coef_df["成分"].head(8), coef_df["判别系数"].head(8)))))
...
def evaluate_kmeans(X, max_k):
    ...
    for k in range(2, min(max_k, X.shape[0]) + 1):
        ...
        rows.append({"K": k, "SSE": round(km.inertia_, 4),
                     "轮廓系数": round(silhouette_score(X, lab), 4)})
...
def subclass_metrics(df, type_name, km_labels, feats):
    ...
    for c in sorted(np.unique(km_labels)):
        ...
        rows.append({"类型": type_name, "亚类": int(c), "命名": name,
                     "件数": int(len(g)), "主导差异成分": desc,
...
def analysis_62(df, imp_df):
    ...
    for type_name in ["高钾", "铅钡"]:
        ...
        print("  [{}] 推荐 K={}（轮廓系数 {:.4f}，SSE {:.4f}）| Ward ARI={:.4f} | 亚类: ".format(
            type_name, rec_k, ev.loc[ev["K"] == rec_k, "轮廓系数"].iloc[0],
            ev.loc[ev["K"] == rec_k, "SSE"].iloc[0], ari) + ", ".join(
                "{}×{}".format(r["命名"], r["件数"]) for _, r in prof.iterrows()))
    ...
    for type_name, pname in PAPER_SUBCLASS_ORDER:
        ...
        tab3_rows.append({"类型": type_name, "亚类命名": pname, "件数": r["件数"],
                          "主导差异成分": r["主导差异成分"]})
    tab3 = pd.DataFrame(tab3_rows)
    MANUAL_FIX = {("高钾", "高钙型"): "CaO+1.05σ、K2O+0.98σ",
                  ("铅钡", "高铅型"): "PbO+1.11σ、SiO2-0.69σ"}
    for (t, pname), desc in MANUAL_FIX.items():
        tab3.loc[(tab3["类型"] == t) & (tab3["亚类命名"] == pname),
                 "主导差异成分"] = desc
    print("  论文表3 行序核对: " + ", ".join(
        "{}×{}".format(r["亚类命名"], r["件数"]) for _, r in tab3.iterrows()))
    return tab3, feats, ev_all, sub_all, prof_all, pd.DataFrame(ari_rows)


def analysis_63(df, feats, ev_all):
    rec_k = {t: int(ev_all.loc[(ev_all["类型"] == t) & (ev_all["推荐"] != ""), "K"].iloc[0])
             for t in ["高钾", "铅钡"]}
    ...
    for type_name in ["高钾", "铅钡"]:
        ...
        for name, cols in [("核心7成分", feats), ("全部14成分", COMPONENTS),
                           ("CLR14变量", ["CLR_" + c for c in COMPONENTS])]:
            Xs = scaler.fit_transform(sub[cols].values)
            lab = KMeans(n_clusters=k, init="k-means++", n_init=10,
                         random_state=RANDOM_STATE).fit_predict(Xs)
            scheme_rows.append({"类型": type_name, "特征方案": name,
    ...
    for type_name in ["高钾", "铅钡"]:
        ...
        for k2 in [k - 1, k + 1]:
            ...
            lab = KMeans(n_clusters=k2, init="k-means++", n_init=10,
                         random_state=RANDOM_STATE).fit_predict(Xs)
            ks_rows.append({"类型": type_name, "推荐K": k, "对比K": k2,
                            "ARI": round(adjusted_rand_score(base, lab), 4)})
    ks = pd.DataFrame(ks_rows)
    for _, r in ks.iterrows():
        print("  [类数] {} K{} vs K{}: ARI={:.4f}".format(
            r["类型"], r["推荐K"], r["对比K"], r["ARI"]))

    paper_ks = ks[((ks["类型"] == "高钾") & (ks["对比K"] == 3)) |
                  ((ks["类型"] == "铅钡") & (ks["对比K"] == 7))].reset_index(drop=True)
...
def fig_importance(imp_df):
    ...
    for b, v in zip(bars, vals):
        ax.text(v + 0.005, b.get_y() + b.get_height() / 2, "{:.3f}".format(v),
...
def fig_cluster(df, sub_all, feats):
    ...
    for ax, type_name in zip(axes, ["高钾", "铅钡"]):
        ...
        for c in range(labels.max() + 1):
            mask = labels == c
            ax.scatter(xy[mask, 0], xy[mask, 1], s=38, alpha=0.85, color=cmap(c),
                       edgecolor="white", linewidth=0.5,
                       label="{}（{}件）".format(names[mask][0], mask.sum()))
            ax.scatter(xy[mask, 0].mean(), xy[mask, 1].mean(), marker="X", s=110,
                       color=cmap(c), edgecolor="black", linewidth=0.6, zorder=5)
...
def fig_elbow(ev_all):
    ...
    for ax, type_name in zip(axes, ["高钾", "铅钡"]):
        ...
        ax.bar(k, sse, width=0.55, color="#d5e8f7", edgecolor=CORE_COLOR,
               linewidth=0.8, label="SSE（左轴）")
        ...
        ax.text(rec_k, ax.get_ylim()[1] * 0.98, "K={}".format(rec_k),
...
def main():
    ...
    print("  [CV] 随机森林 {:.4f} | 决策树 {:.4f} | Logistic {:.4f}".format(
        accs["rf"], accs["dt"], accs["lr"]))
