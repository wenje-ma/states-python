...
def analysis_51(df_form1):
    ...
    for col in ["类型", "纹饰", "颜色系"]:
        ...
        rho, tau, n = rank_correlations(df, col,
            {"类型": TYPE_CODE, "纹饰": ORNA_CODE, "颜色系": COLOR_CODE}[col])
        sig = "显著相关" if p < 0.05 else "不显著"
        rows.append({
            "属性": col, "样本数": n,
            "卡方统计量": round(chi2, 3), "P值": round(p, 4),
            "检验方式": "蒙特卡洛置换" if use_perm else "卡方近似",
            "Spearman ρ": round(rho, 3), "Kendall τ": round(tau, 3),
            "结论": sig,
        })
        print("  [{}] 卡方={:.3f} P={:.4f}（{}）| ρ={:+.3f} τ={:+.3f} → {}".format(
            col, chi2, p, "置换" if use_perm else "卡方", rho, tau, sig))
    ...
    share_df = pd.DataFrame({
        "类型": list(share.keys()),
        "风化占比%": [round(share[t], 1) for t in share],
    })
...
def analysis_52(art):
    ...
    for t in ["高钾", "铅钡"]:
        ...
        for col in COMPONENTS:
            ...
            all_tests.append({
                "类型": t, "成分": col,
                "无风化均值%": round(g0.mean(), 2), "风化均值%": round(g1.mean(), 2),
                "P值": round(p, 4), "Cohen_d": round(d, 3),
                "变化方向": "上升" if g1.mean() > g0.mean() else "下降",
                "显著性": "显著" if p < ALPHA else "不显著",
                "n无风化": len(g0), "n风化": len(g1),
            })
    tests = pd.DataFrame(all_tests)
    sig = tests[tests["显著性"] == "显著"]
    print("  显著成分（P<0.05）: " + ", ".join(
        "{}:{}".format(t, c) for t, c in zip(sig["类型"], sig["成分"])))

    PAPER_COMPS = {"高钾": ["SiO2", "K2O", "CaO", "Al2O3", "Fe2O3"],
                   "铅钡": ["SiO2", "PbO", "CaO", "P2O5"]}
    paper_rows = []
...
def analysis_53(df):
    print("  有效采样点 {} 个（未风化 {} / 一般风化 {} / 严重风化 {}）".format(
        len(df), (df["Y"] == 0).sum(), df["Y"].isin([0.75, 0.8]).sum(), (df["Y"] == 1).sum()))

    func_rows, ratio_rows, pred_notes = [], [], []
    for t in ["高钾", "铅钡"]:
        sub = df[df["类型"] == t]
        predA, sumA = method_A(sub, MODEL_COMPS[t])
        print("\n  [{}玻璃] 主线A：R²={:.4f} | 系数 {} | 反推 {} 个风化样本".format(
            t, sumA["R2"], sumA["系数"], len(predA)))
        ...
        for c in MODEL_COMPS[t]:
            ratio_rows.append({"类型": t, "成分": c,
                               "反应比例": sumA["反应比例"][c]})
        print("    反应比例: " + ", ".join(
            "{}={}".format(c, sumA["反应比例"][c]) for c in MODEL_COMPS[t]))
        ...
        if len(predA):
            ...
            check = check.dropna()
            check["标准化偏差"] = ((check["预测均值"] - check["无风化均值"]).abs()
                                  / check["无风化标准差"]).round(3)
            ...
            print("    标准化偏差：全部 {}；除高缺失组分外 {} （论文口径）".format(
                "通过" if ok_all else "未通过",
                "通过" if ok_low else "未通过"))
            if t == "高钾":
                sio2_lo, sio2_hi = predA["预测风化前SiO2"].min(), predA["预测风化前SiO2"].max()
                k2o_lo, k2o_hi = predA["K2O"].min(), predA["K2O"].max()
                pred_notes.append({"类型": t, "指标": "预测风化前SiO2区间%",
                                   "值": "{:.1f}~{:.1f}".format(sio2_lo, sio2_hi),
                                   "说明": "论文 5.3：{} 个{}风化采样点".format(len(predA), t)})
                pred_notes.append({"类型": t, "指标": "预测风化前K2O区间%",
                                   "值": "{:.1f}~{:.1f}".format(k2o_lo, k2o_hi),
                                   "说明": "论文 5.3 报告值"})
                print("    预测区间：SiO2 {:.1f}~{:.1f} | K2O {:.1f}~{:.1f}".format(
                    sio2_lo, sio2_hi, k2o_lo, k2o_hi))

            p08 = predA[(predA["文物编号"] == 8) & (predA["Y"] == 1.0)]
            if len(p08):
                v = p08["预测风化前SiO2"].iloc[0]
                pred_notes.append({"类型": t, "指标": "严重风化样本08原始SiO2预测%",
                                   "值": "{:.2f}".format(v),
                                   "说明": "论文 5.3：采样点 08严重风化点"})
    ...
    for t in ["高钾", "铅钡"]:
        ...
        for k, v in sumB.items():
            if isinstance(v, dict) and "R2" in v:
                ...
                if (t, k) in PAPER_B:
                    pred_notes.append({
                        "类型": t, "指标": "对照B {}模型".format(k),
                        "值": "R²={}，WG系数={}".format(v["R2"], v["系数"].get("WG")),
                        "说明": "论文 5.3：风化虚拟变量回归对照"})
    return (pd.DataFrame(func_rows), pd.DataFrame(ratio_rows),
            pd.DataFrame(pred_notes))
