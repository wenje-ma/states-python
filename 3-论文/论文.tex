...
\title{基于成分数据建模与多模型融合的 \\ 古代玻璃制品成分分析与鉴别}
\author{赵宇翔 \and 李传浩 \and 马文杰}
\begin{document}

\maketitle

\begin{abstract}
本文针对古代玻璃制品的成分分析与鉴别问题，围绕成分数据定和约束、检出限缺失与风化扰动三大难点，建立了"数据预处理—风化规律建模—分类聚类—未知样本判别"的完整框架，并对结论进行了稳健性检验。

\textbf{针对问题一：} 以 $85\%\sim105\%$ 累加和标准筛选有效数据，对中缺失成分采用 \textbf{SVR 插补}，经采样点聚合与 \textbf{ALR / CLR 对数比变换}消除成分伪相关。运用\textbf{卡方检验与蒙特卡洛置换检验方法}，结合 \textbf{Spearman、Kendall 秩相关}，得出风化与玻璃类型显著相关，与纹饰、颜色无显著关联的结论；运用 \textbf{Mann-Whitney U 检验与 Cohen's d 效应量}，结果显示高钾风化后 $\mathrm{SiO_2}$ 富集、铅钡 $\mathrm{SiO_2}$ 流失。进一步构建风化程度回归并估计反应比例，以此来反推风化前成分，并运用虚拟变量回归对照验证。

\textbf{针对问题二：} 以随机森林特征重要性筛选前 $7$ 位核心成分，构建\textbf{逻辑回归、随机森林与深度受限决策树}三判别模型，并用分层 $5$ 折交叉验证评估三模型泛化能力；两类玻璃经 \textbf{K-means++ 聚类}得高钾 $K=4$、铅钡 $K=6$ 个亚类，ARI 与 $\pm5\%$、$\pm10\%$ 的扰动检验及 \textbf{Ward 对照}验证了划分的合理性与稳健性。

\textbf{针对问题三：} 运用问题二的判别底座对 $8$ 件未知样品判别，风化样本经\textbf{假设-验证}还原风化前成分后再判别，判定 $\mathrm{A}1$、$\mathrm{A}6$、$\mathrm{A}7$ 为高钾，$\mathrm{A}2$、$\mathrm{A}3$、$\mathrm{A}4$、$\mathrm{A}5$、$\mathrm{A}8$ 为铅钡；$\pm5\%$、$\pm10\%$ 扰动下 $100$ 次判别类别翻转率均为 $0$，结果稳健可靠。
\end{abstract}

\textbf{关键词}：古代玻璃；成分数据分析；SVR 插补；风化前成分反推；随机森林；K-means++；敏感性分析
...
本文数据集为典型氧化物成分数据，受定和闭合约束与仪器检出限影响，普遍存在成分未检出、成分总和偏离 $100\%$ 的数据失真问题。为消除成分数据单形空间伪相关性、适配常规统计与机器学习建模，本文参照已有古玻璃数据处理范式（彭丽红等，2024；侯兴汉等，2026）\cite{penglihong2024,houxinghan2026}开展系统化预处理。首先通过正则解析拆分原始复合采样点字段，完成多表属性关联，以成分累加和 $S_i$ 作为有效性判定依据。
...
其中 $S_i$ 为第 $i$ 个采样点的成分累加和，$x_{id}$ 为该采样点第 $d$ 种成分的质量分数，$D$ 为成分总数。参照彭丽红等（2024），\textbf{限定化学成分比例累加和在 $85\%\sim105\%$ 才可视为有效数据}，以 $V_{i}=\mathbf{1}\{85\%\le S_{i}\le105\%\}$ 标记样本有效性，剔除 $2$ 条无效采样数据，保留 $67$ 条已知、$8$ 条未知有效样本。

\subsection{缺失率统计与缺失机制判别}
...
\subsubsection{SVR 插补}

针对中缺失成分，本文参照侯兴汉等（2026）\textbf{提出一种基于滑动窗口的支持向量回归（SVR）插补方法，通过分阶段网格搜索与 $5$ 折交叉验证优化模型超参数}的思路，将之改造适配截面数据，拟合高斯核 SVR 模型
...
本文参照钱鸿羽等（2024）\cite{qianhongyu2024}为了消除定和约束的影响，通过计算各个组别所占成分比例与其均值的比值的对数，将成分数据从单形空间变换到欧式空间，从而能够应用经典统计方法解决成分数据问题的处理方法，对 $8$ 组可靠成分开展对数比变换。先采用极小值替换规避对数无定义问题，再分别构建以 $\mathrm{SiO_2}$ 为基准的加性对数比（ALR）
...
本文做出如下模型假设：未风化文物以及文物未风化部位的检测成分可以近似作为玻璃风化前的原始组分（熊强等，2025），高钾、铅钡玻璃类别在风化前后维持不变；为避免风化引起的化学成分含量变化对判定玻璃体系的影响，从样本数据中选取无风化样本作为数据拟合的内容（钱鸿羽等，2024）；水质数据缺失一般包括完全随机缺失、随机缺失和非随机缺失（侯兴汉等，2026），未检出成分视作低于仪器检出限而非随机缺失；同一类型玻璃发生风化时，由于在化学反应中涉及的化学成分是成比例的（钱鸿羽等，2024），各成分迁移、反应的比例具备相对稳定性，可依托各成分相对于 $\mathrm{SiO_2}$ 的反应比例描述风化行为；风化程度以 $0\sim1$ 区间连续变量量化（钱鸿羽等，2024）；不同文物样本相互独立，同一文物的多个采样点经聚合后视为单个独立样本，可代表该文物整体成分特征（课件，第 8 讲 - 独立性）。

\section{问题 1 模型}

\subsection{表面风化与玻璃类型、纹饰、颜色的关系}

\subsubsection{模型讲解}

彭丽红等（2024）在同类古玻璃风化研究中运用卡方独立性检验来探讨影响玻璃文物表面风化的因素，并指出进行独立性检验时，若列联表中有单元格频数为零，取 Fisher 精确概率检验结果，否则取卡方检验结果。本文纹饰、颜色的列联表存在零频数与期望频数过小的问题，因此采用蒙特卡洛置换检验作为 Fisher 精确检验的扩展方案；同时参照熊强等（2025）\cite{xiongqiang2025}为更精确地量化各因子与风化情况的相关性，采用 Spearman 秩相关系数来分析、刘振文等（2025）\cite{liuzhenwen2025}使用 Kendall tau 系数及其 P 值探究之间的相关性和差异性的思路，开展非参数关联分析，实现多方法交叉验证。


\subsubsection{卡方独立性检验与蒙特卡洛置换检验}
...
\begin{equation*}
P_{\mathrm{perm}}=\frac{\sum_{t=1}^{N_{\mathrm{perm}}}\mathbf{1}\{\chi_{t}^{2}\ge\chi_{\mathrm{obs}}^{2}\}+1}{N_{\mathrm{perm}}+1},
\end{equation*}

分子分母同加 $1$ 以避免 $P=0$ 的伪精确；该检验可视作费希尔精确检验对 $L\times2$ 列联表的推广。


\subsubsection{Spearman 与 Kendall 秩相关}
...
基于 $58$ 件文物级样本开展计算，颜色分析剔除 $4$ 条颜色缺失样本并归并色系，所得统计结果如表 \ref{tab:assoc} 所示。

\begin{table}[htbp]
\centering
\caption{表面风化与玻璃类型、纹饰、颜色关联性的检验结果}
\label{tab:assoc}
...
钱鸿羽等（2024）对化学成分含量进行正态性检验，发现总体不符合正态分布，因此选择非参数检验中的独立样本 Mann-Whitney U 检验，按照 Cohen's d 系数排序分析具有显著性差异的化学成分。本文数据集各成分同样偏离正态分布，直接复用该检验框架；同时参考熊强等（2025）将高钾玻璃分为未风化和风化两类，铅钡玻璃分为未风化、风化和严重风化三类的处理方式，分玻璃类型对比风化、未风化样本的成分统计特征。具体以文物级主表（$51$ 件）为数据口径、按类型$\times$风化分组；因部分成分存在未检出，各组各成分的有效样本数以实际检出数为准（不足 $2$ 个时不做检验，如高钾玻璃中多数风化样本未检出的成分）。
...
其含义等价于风化组中每个观测大于无风化组观测的次数之和。在 $H_0$ 下 $U$ 的分布仅取决于 $n_{0}$、$n_{1}$，其中小样本取精确分布，大样本可用正态近似，据此计算双尾 P 值；显著性水平取 $\alpha=0.05$，$P<\alpha$ 时认为该成分含量在风化前后存在显著差异。


\subsubsection{Cohen's d 效应量}
...
对两类玻璃的各成分逐对检验，即可结合显著性、$\mathrm{d}$ 的符号与大小归纳风化前后的含量统计规律。高钾玻璃中，风化后 $\mathrm{SiO_2}$ 均值由 $66.69\%$ 上升至 $93.96\%$（$\mathrm{P}<0.001$，Cohen's $\mathrm{d}=4.80$），$\mathrm{K_2O}$、$\mathrm{\mathrm{CaO}}$、$\mathrm{Al_2O_3}$、$\mathrm{Fe_2O_3}$ 均出现显著下降。铅钡玻璃表现出相反变化趋势：$\mathrm{SiO_2}$ 由 $54.93\%$ 下降至 $27.73\%$（$\mathrm{P}<0.001$，$\mathrm{d}=-2.45$），$\mathrm{\mathrm{PbO}}$、$\mathrm{\mathrm{CaO}}$、$\mathrm{P_2O_5}$ 含量显著升高。高钾玻璃风化过程表现为二氧化硅富集、助熔与碱金属组分流失；铅钡玻璃则发生二氧化硅流失、重金属氧化物相对富集。

\subsection{风化前化学成分预测}

\subsubsection{模型讲解}

借鉴钱鸿羽等（2024）的建模思路：构建风化程度多元函数，依据风化过程的成分成比例迁移特性估计各组分反应比例，反算风化损耗量，风化前的数据可以通过从风化后的数据中减去变化来获得，进而还原风化前原始成分；同时引入熊强等（2025）将采样部位的风化程度作为核心解释变量，建立包含定性变量的多元线性回归方程进行分析的回归模型作为对照，校验预测趋势合理性。

建模以有效采样点为单元，记风化样本 $i$ 第 $d$ 种成分的实测含量为 $x_{id}$，未风化点及无风化样本的检测值视为风化前的近似基准，风化前成分反推分三步：建立风化程度函数、估计风化反应比例、反算风化损耗量还原原始成分。


\subsubsection{风化程度多元回归}
...
回归系数 $(\beta_0,\beta_d)$ 由最小二乘法确定，即使残差平方和最小，$\widehat{\beta}_0$、$\widehat{\beta}_d$ 即其最小二乘估计值

\begin{equation*}
(\widehat\beta_{0},\widehat\beta_{d})=\arg\min\sum_{i}(Y_i-\beta_{0}-\sum_{d\in\mathcal{S}}\beta_{d}x_{id})^{2}
\end{equation*}

$R^2$ 反映所选成分对风化程度的可解释比例。


\subsubsection{风化反应比例估计}

第二步，估计各成分相对 $\mathrm{SiO_2}$ 的风化反应比例。记类型内无风化组、风化组第 $d$ 种成分的均值分别为 $\overline{x}_{0,d}$、$\overline{x}_{1,d}$（与 5.1、5.2 的 $0$ 为无风化、$1$ 为风化编码一致），风化引起的含量差为 $\Delta_{d}=\overline{x}_{1,d}-\overline{x}_{0,d}$，则反应比例
...
\subsubsection{风化前成分反推}

第三步，反推风化前成分。设风化引起的各成分变化按反应比例成比例迁移，即 $\Delta x_{d}=r_{d}\Delta x_{\mathrm{SiO_2}}$；将回归式写成关于 $\Delta x_{\mathrm{SiO_2}}$ 的线性增量并与风化程度 $Y_i$ 对应，可解出 $\mathrm{SiO_2}$ 的变化量，进而还原各成分风化前含量估计 $\widehat{x}_{id}$

\begin{equation*}
\widehat{x}_{id}=x_{id}-\frac{r_{d}}{A}Y_i,\quad A=\beta_{\mathrm{SiO_2}}+\sum_{d\in\mathcal{S}\setminus\{\mathrm{SiO_2}\}}\beta_{d}r_{d}
\end{equation*}
...
\subsubsection{标准化偏差检验}

预测合理性以标准化偏差检验：设反推所得风化前含量的均值为 $\overline{\widehat{x}}_{d}$，与无风化组均值 $\overline{x}_{0,d}$、样本标准差 $s_{0,d}$ 比较

\begin{equation*}
\frac{|\overline{\widehat{x}}_{d}-\overline{x}_{0,d}|}{s_{0,d}}<1
\end{equation*}

小于 $1$ 表示反推结果落在无风化组均值一倍标准差之内，预测与未风化样本分布一致。


\subsubsection{风化虚拟变量回归对照}

作为对照，按熊强等（2025）的风化虚拟变量回归复核趋势方向：对风化显著相关成分 $d$，以少量低缺失成分为特征集 $\mathcal{F}_d$ 拟合

\begin{equation*}
x_{id}=\alpha_{d}+\sum_{f\in\mathcal{F}_d}\beta_{df}x_{if}+\lambda_{d}WG_i,\quad WG_i=\mathbf{1}\{Y_i>0\}
\end{equation*}
...
其中 $Y_K$ 为高钾玻璃的风化程度函数取值，$x_{\mathrm{SiO_2}}$、$x_{\mathrm{K_2O}}$、$x_{\mathrm{CaO}}$、$x_{\mathrm{Al_2O_3}}$ 为对应成分的实测含量，$R^2=0.862$；
...
\subsection{高钾、铅钡玻璃的分类规律}

\subsubsection{模型讲解}

张星园等（2023）\cite{zhangxingyuan2023}通过综合相关性分析研究两种玻璃的分类规律，发现玻璃类型和二氧化硅、氧化钾、氧化铅和氧化钡的相关系数可达到 $0.7$。本文先以 Spearman 秩相关刻画类型与各成分的关联强度，再按刘振文等（2025）使用网格寻优对 RF 超参数进行调节，以此来获得影响样本分类的 $7$ 个重要指标进行数据降维的思路，以随机森林特征重要性筛选分类核心成分，同时拟合逻辑回归判别函数（钱鸿羽等，2024），并以深度受限决策树输出可读分类规则（熊强等，2025）；各模型准确率均采用分层 $5$ 折交叉验证报告，标准化参数仅在训练折内估计，避免数据泄漏（课件，第3讲 - K 折交叉验证；课件，第3讲 - 数据泄漏）\cite{kejian2026}。


\subsubsection{特征标准化与 Spearman 秩相关}

设以预处理后的 $56$ 件有效文物级样本为建模对象，类型编码为二值标签 $y_i$（高钾 $y_i=0$、铅钡 $y_i=1$，与表 \ref{tab:core} 注一致）；第 $i$ 个样本第 $d$ 种成分含量 $C_{id}$（即 $C_{\mathrm{SiO_2}}=C_{i,\mathrm{SiO_2}}$ 一类的含量记号，化学式下标同 5.3）尺度差异大，建模前先做 $Z$ 分数标准化
...
其中 $\eta_i$ 为标准化成分的线性组合（对数几率）、$a_0$ 为截距项；判别系数 $a_d$（表 \ref{tab:core}）为正促进判铅钡、为负促进判高钾，$P_i>0.5$ 判为铅钡。

\subsubsection{CART 决策树}

其三，深度受限（深度 $3$）的 CART 决策树同样以基尼下降最大选择划分，叶节点取多数类，输出“$\mathrm{PbO}\le5.46$ 判高钾、$\mathrm{PbO}>5.46$ 判铅钡”的可读规则（熊强等，2025）。


\subsubsection{分层 5 折交叉验证}

三模型的泛化能力均以分层 $5$ 折交叉验证报告：按类别比例分层将 $n$ 个样本分为 $5$ 折，逐折以其余 $4$ 折训练、该折测试，折外准确率

\begin{equation*}
\mathrm{Acc}=\frac{1}{n}\sum_{i=1}^{n}\mathrm{1}\{\widehat{y}_{i}=y_{i}\}
\end{equation*}
...
\begin{table}[htbp]
\centering
\caption{核心成分的 Spearman 相关、随机森林特征重要性与逻辑回归判别系数}
\label{tab:core}
\begin{tabular}{c|c|c|c}
\hline
成分 & Spearman $\rho$ & RF 特征重要性 & Logistic 判别系数 \\
\hline
...
表 \ref{tab:core} 显示，$\mathrm{PbO}$、$\mathrm{BaO}$ 与铅钡类型强正相关（$\rho>0.75$）且 RF 重要性居前两位，$\mathrm{SiO_2}$、$\mathrm{K_2O}$ 与高钾类型强负相关，Logistic 判别系数方向与之一致，四者共同构成判别两类玻璃的核心成分。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{figures/fig_q2_importance.pdf}
\caption{14 种成分的随机森林特征重要性}
\label{fig:importance}
\end{figure}

图 \ref{fig:importance} 显示 $14$ 种成分的重要性呈长尾分布，前 $7$ 位累计贡献 $91.6\%$，$\mathrm{PbO}$ 单项即占 $27.3\%$，说明类型信息高度集中于少数核心成分。

\subsection{两类玻璃内部的亚类划分}
...
其中 $\bar{C}_{k,d}$ 为簇 $k$ 在成分 $d$ 上的均值、$\bar{C}_{d}$ 为同类型全体样本的均值；以同类型样本标准差 $s_d$ 为单位度量（表注中以 $\sigma$ 表示），$|t_{kd}|$ 明显大于 $1$ 的成分即该亚类的主导差异成分（表 \ref{tab:subclass}），据此按玻璃化学特征命名亚类（彭丽红等，2024）。
...
表 \ref{tab:subclass} 表明，高钾玻璃四个亚类由钙、钡、铅等组分的富集或亏损区分；铅钡玻璃六个亚类以硅、铅、磷、铜、铝、钾为主导，其中高硅型（$15$ 件）与高铅型（$9$ 件）为主体亚类，分别对应硅基与重铅基配方体系。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{figures/fig_q2_cluster.pdf}
\caption{高钾、铅钡玻璃亚类划分的 PCA 投影}
\label{fig:cluster}
\end{figure}

图 \ref{fig:cluster} 中两类型样本在 PCA 投影下按亚类自然成团、质心分离清晰；高钾样本稀疏、铅钡样本密集，铅钡的高硅型与高铅型沿第一主成分明显分开。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{figures/fig_q2_elbow.pdf}
\caption{两类玻璃的轮廓系数与 SSE 随类数变化}
\label{fig:elbow}
\end{figure}

图 \ref{fig:elbow} 显示两类玻璃的轮廓系数分别在 $K=4$ 与 $K=6$ 处达到峰值，SSE 此后下降趋缓，说明推荐类数兼顾了簇内紧致度与簇间分离度。

\subsection{亚类划分的合理性与敏感性}
...
\subsubsection{乘性高斯扰动敏感性检验}

敏感性检验对每件样本的核心成分施加同幅度乘性高斯扰动

\begin{equation*}
C'_{id}=C_{id}\,(1+\epsilon_{id}),\quad \epsilon_{id}\sim N(0,\delta^{2}),
\end{equation*}
...
\begin{equation*}
\widehat{C}_{id}^{(h)}=C_{id}-\frac{r_{d}^{(h)}}{A^{(h)}}\,Y_{i},
\end{equation*}
...
\subsubsection{直接判别与多数表决}

对无风化样本，判别输入即观测成分。设逻辑回归给出的铅钡概率为 $P_i$（同 7.1），随机森林与决策树各投一票，则样本类别与置信度分别为

\begin{equation*}
\widehat y_i=\begin{cases}\text{铅钡},&P_i>0.5,\\\text{高钾},&P_i\le0.5,\end{cases}\quad c_i=|P_i-0.5|\times2,
\end{equation*}

$c_i\in[0,1]$ 反映概率偏离判界 $0.5$ 的程度，越大判别越可信；三模型多数表决作为最终类别的一致性依据。


\subsubsection{假设-验证综合判定}
...
$8$ 件未知样品的判别依据与结果如表 \ref{tab:unknown} 所示。

\begin{table}[htbp]
\centering
\caption{8 件未知样品的判别依据与结果}
\label{tab:unknown}
\resizebox{\textwidth}{!}{
\begin{tabular}{c|c|c|c|c|c|c}
\hline
样品 & 表面风化 & 直接判别P(铅钡) & 高钾假设还原P(铅钡) & 铅钡假设还原P(铅钡) & 最终类型 & 置信度 \\
\hline
A1 & 无风化 & $0.14$ & — & — & 高钾 & $0.72$ \\
A2 & 风化 & $0.89$ & — & — & 铅钡 & $0.79$ \\
A3 & 无风化 & $0.98$ & — & — & 铅钡 & $0.95$ \\
...
表 \ref{tab:unknown} 显示，A1、A6、A7 判为高钾，A2、A3、A4、A5、A8 判为铅钡。未风化样本 A1、A3、A4、A8 的成分即为风化前近似，直接判别置信度均不低于 $0.72$；风化样本 A6、A7 的高钾假设还原完全自洽，还原后 $\mathrm{SiO_2}$ 约 $58\%$、$\mathrm{K_2O}$ 约 $12.7\%$，与未风化高钾样本画像一致，置信度超过 $0.97$；A2、A5 的检出成分过少或还原结果超出物理范围，假设-验证不可行，但直接判别三模型投票一致且置信度高于 $0.78$，结论可靠。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{figures/fig_q3_prob.pdf}
\caption{8 件未知样品的判别概率}
\label{fig:prob}
\end{figure}

图 \ref{fig:prob} 显示，$8$ 件样本的判别概率全部远离 $0.5$ 判界，不存在临界样本；高钾样本概率集中在 $0.11\sim0.17$，铅钡样本集中在 $0.89\sim0.98$，两类判别的概率区间完全分离。

判别结果与熊强等（2025）对未知样品的判别在 $7/8$ 件上一致。唯一分歧为 A8：熊强等（2025）将其判为高钾，但对应概率仅为 $0.925$，明显低于其判定的其余高钾样本；本文直接判别三模型一致判为铅钡。A8 为无风化样本，$\mathrm{PbO}=21.24\%$、$\mathrm{BaO}=11.34\%$，正落在已知铅钡未风化组的中位水平，而高钾未风化组 $\mathrm{PbO}$ 均值仅 $0.35\%$，从核心判别成分看 A8 的化学证据强烈支持铅钡；且 A8 的 $\mathrm{SiO_2}=51.1\%$，与熊强等划入同一亚类的 A6、A7 差异悬殊。综合直接判别的三模型一致性、无风化样本原始成分的可信度与铅钡核心成分区间，本文认为 A8 判为铅钡更为合理。

\subsection{判别结果的敏感性分析}

\subsubsection{模型讲解}

为检验判别结果对成分测量扰动的稳健性，沿用熊强等（2025）敏感度分析中将各个文物样本的化合物含量参数正向调整 $5\%$ 及 $10\%$，并在调整后的数值上重新进行判别，若调整后仍旧在原分类中，则可证明模型的鲁棒性的思路，对每件未知样本的判别输入施加 $\pm5\%$、$\pm10\%$ 的乘性高斯扰动并重复 $100$ 次，统计类别翻转率。


\subsubsection{乘性高斯扰动与类别翻转率}
...
$\delta$ 取 $5\%$ 与 $10\%$，每次扰动后重新标准化并判别，重复 $100$ 次；记基准判别为 $\widehat y_i$、第 $m$ 次扰动判别为 $\widehat y_i^{(m)}$，则该样本的类别翻转率为

\begin{equation*}
f_i=\frac{1}{100}\sum_{m=1}^{100}\mathrm{1}\big\{\widehat y_i^{(m)}\neq \widehat y_i\big\},
\end{equation*}
...
结果如表 \ref{tab:flip} 所示。

\begin{table}[htbp]
\centering
\caption{未知样品在 $\pm5\%$、$\pm10\%$ 扰动下的类别翻转率}
\label{tab:flip}
\begin{tabular}{c|c|c}
\hline
样品 & $\pm5\%$ 扰动翻转率 & $\pm10\%$ 扰动翻转率 \\
\hline
A1 & $0$ & $0$ \\
A2 & $0$ & $0$ \\
A3 & $0$ & $0$ \\
...
表 \ref{tab:flip} 显示，$8$ 件未知样本在 $\pm5\%$、$\pm10\%$ 扰动下的类别翻转率均为 $0$，判别结果对成分测量误差不敏感；结合判别概率远离判界以及与文献判别结果的高度一致，未知样品鉴别结论稳健可靠。

\section{优缺点}

优点方面，其一，数据处理贴合成分数据的数学结构：未检出按低于检出限处理、只对存在可解释关联的中等缺失成分做 SVR 插补、以 ALR / CLR 对数比变换解除定和约束、标准化参数仅用训练折，从源头降低了伪相关与分布失真。

其二，结论多方法互证且可解释：关联与差异分析兼用非参数检验与效应量，分类规律由相关分析、随机森林特征重要性、逻辑回归与浅决策树相互印证，并给出 $\mathrm{PbO}\le5.46$ 一类显式规则，聚类亦以 K-means++ 与 Ward 层次聚类对照。

其三，评估与稳健性链条完整：模型用分层 $5$ 折交叉验证报告精度，亚类划分经 ARI、扰动与类数、特征方案敏感性检验，未知样品经 $\pm5\%$、$\pm10\%$ 扰动翻转率检验，风化样品走“假设-验证”闭环，结论有较充分的证据支撑。

缺点方面，其一，线性模型（逻辑回归与多元回归）依赖线性结构与近似正态假设，成分间存在共线性且样本量小，回归系数可能不稳定，判别边界非线性时表达能力有限。

其二，聚类与统计检验有固有限制：K-means++ 较适合凸形簇、需预设类数，轮廓系数在簇边界重叠时选 K 不稳；多成分、多组的多重比较未做校正，小样本下 Mann-Whitney U 与 Cohen's d 的检验功效受限。

其三，风化前还原依赖“反应成比例迁移 + 风化程度主观赋值”的假设，高钾玻璃缺少严重风化样本使其风化程度函数仅有两档、外推有限；SVR 插补值误差会进入下游、高缺失成分仅保留二值信息，且整体基于 $56$ 件小样本，结论外推需谨慎。
...
\subsection{数据预处理（\normalfont\texttt{预处理.py}）}
\label{app:pre}

{\linespread{1}\selectfont
\lstinputlisting{../2-解答/0-数据预处理/预处理.py}
}

\subsection{问题 1 求解（\normalfont\texttt{第一问.py}）}
\label{app:q1}

{\linespread{1}\selectfont
\lstinputlisting{../2-解答/1-第一问/第一问.py}
}

\subsection{问题 2 求解（\normalfont\texttt{第二问.py}）}
\label{app:q2}

{\linespread{1}\selectfont
\lstinputlisting{../2-解答/2-第二问/第二问.py}
}

\subsection{问题 3 求解（\normalfont\texttt{第三问.py}）}
\label{app:q3}

{\linespread{1}\selectfont
\lstinputlisting{../2-解答/3-第三问/第三问.py}
}
